...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::HorizonScript()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
	{
		static const char* const cppKeywords[] = {
			"void", "float", "int", "int64", "string", "bool", "index", "char",
			"if", "else if", "else", "for_loop", "for_each", "key", "hprint"
		};
		for (auto& k : cppKeywords)
			langDef.mKeywords.insert(k);

		static const char* const identifiers[] = {
			"list", "map", "pair", "add", "remove_index", "find", "append",
			"remove"
		};
		for (auto& k : identifiers)
		{
